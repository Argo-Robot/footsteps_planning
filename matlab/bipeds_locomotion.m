...
function [trajectory] = generate_global_trajectory(v,w,T, delta_T)
    ...
    % Inizializzazione posizione CoM (partiamo da zero)
    pos_x = 0.0; 
    pos_y = 0.0;
    pos_z = 1.0;
    theta = 0.0;
...
function foot_positions = generate_foot_trajectory_2d(trajectory, step_length, step_width, foot_size)
    ...
    % trajectory: Nx2 matrix of global trajectory points [x, y]
    % step_length: distanza tra passi consecutivi lungo la tangente
    % step_width: distanza laterale tra i piedi
    % delta_dist: risoluzione della traiettoria originale
    % foot_size: dimensione del piede [lunghezza, larghezza]

    num_points = size(trajectory, 1);
    foot_positions = []; % Per salvare le posizioni e orientamenti [x, y, theta, is_right]
    ...
    for i = 2:num_points-1
        ...
        if dist_accumulated >= step_length
            dist_accumulated = 0; % Resetta la distanza accumulata
    ...
    for i = 1:size(foot_positions, 1)
        % Disegna i rettangoli dei piedi con il corretto orientamento
    ...
    % Disegna la traiettoria originale
...
function foot_trajectory = generate_foot_trajectory_3d(foot_positions, step_height, step_length, step_time, delta_T)
    ...
    foot_trajectory = cell(size(foot_positions, 1) - 2, 1); % Traiettoria per ciascun passo

    for i = 1:size(foot_positions, 1) - 2
        % Punti iniziale e finale del passo
        p_start = foot_positions(i, 1:2); % [x, y] iniziali
        p_end = foot_positions(i + 2, 1:2); % [x, y] finali
        
        % Interpolazione di x e y con spline cubica (per andatura più fluida)
        x_traj = interp1([0, 1], [p_start(1), p_end(1)], t, 'pchip');
        y_traj = interp1([0, 1], [p_start(2), p_end(2)], t, 'pchip');
        
        % Altezza del piede (parabolica)
        z_traj = 4 * step_height * t .* (1 - t); % Profilo parabolico
        
        % Costruzione delle trasformazioni omogenee
        ...
        % Salva la traiettoria del passo
