...
phi_rear_left_0 = 0;          % Rear left leg initial phase
...
% Distance between diagonal legs in the horizontal plane
offset_x = 0.2; % Forward/backward offset [meters]
...
xlim([-0.4, 0.4]);
ylim([-0.2, 0.2]);
xlabel('Forward direction (H_x) [m]');
ylabel('Vertical direction (H_z) [m]');
title('Foot Trajectory Animation (Trot Gait)');
rear_left_plot = plot(0, 0, 'go', 'MarkerSize', 8, 'MarkerFaceColor', 'g');
front_right_plot = plot(0, 0, 'ro', 'MarkerSize', 8, 'MarkerFaceColor', 'r');
trajectory_line_rl = plot(rear_left_traj(1, :), rear_left_traj(2, :), 'g--');
trajectory_line_fr = plot(front_right_traj(1, :), front_right_traj(2, :), 'r--');

% Animate foot trajectories
for i = 1:num_steps
    % Update positions
    set(rear_left_plot, 'XData', rear_left_traj(1, i), 'YData', rear_left_traj(2, i));
